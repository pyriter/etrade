...
export class AuthorizationTokenInterceptor extends Interceptor {
  ...
  async onSuccessRequestHandler(config): Promise<any> {
    if (
      ...
      config.url?.includes(OAUTH2_TOKEN)
    )
      return config;
  ...
  private getAccessType(credentials: Credentials): AccessType {
    const { refresh_token_modified_date, refresh_token_expires_in } =
      credentials;
    const expiredDate =
      refresh_token_modified_date + refresh_token_expires_in * 1000 * 0.9;
    const now = Date.now();
  ...
  private async generateOAuthData(): Promise<OAuthData> {
    ...
    const { client_id, redirect_uri, refresh_token } = credentials;
    const accessType = this.getAccessType(credentials);
    return {
      client_id,
      redirect_uri,
      refresh_token,
  ...
  private async getCredential(): Promise<Credentials> {
    return await this.credentialProvider.getCredential.bind(
      this.credentialProvider
    )();
  }

  private async updateCredential(credentials: Credentials): Promise<void> {
    await this.credentialProvider.updateCredential.bind(
      this.credentialProvider,
      credentials
    )();
  }
